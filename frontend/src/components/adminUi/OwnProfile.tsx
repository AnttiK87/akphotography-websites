...
const OwnProfile = () => {
  ...
  const keywordsFromStore = useAppSelector((state) => state.keywords.keywords);
  const userFromStores = useAppSelector((state) => state.user.user);
  const userData = user ? user : userFromStores ? userFromStores : undefined;
  ...
  const [currentKeyword, setCurrentKeyword] = useState<Keyword | undefined>(
    undefined
  );
  ...
  const deleteKw = (keywordId: number) => {
    if (window.confirm("Do you really want to delete this keyword?")) {
      dispatch(removeKw({ keywordId }, "eng", navigate));
    }
  };

  const handleEditKW = (keyword: Keyword) => {
  ...
  const renderTableBody = () => {
    ...
    return (
      <tbody>
        {sortedKeywords.reduce<React.JSX.Element[]>(
          (acc, keyword, index, sortedKeywords) => {
            if (order === "alpha") {
            ...
            acc.push(
              <tr key={keyword.id}>
                ...
                <td className="vertical-center keywordsOP">
                  {keyword.keyword}
                </td>
