...
const UploadImages = () => {
  ...
  const rawKeywords = useAppSelector((state) => state.keywords.keywords);
  ...
  if (keywordArray && keywordArray.includes(keyword)) {
  ...
  const addPicture = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const yearValue: number | undefined = year ? Number(year) : undefined;
    const textFiValue: string = textFi ?? "";
    const textEnValue: string = textEn ?? "";
    ...
    if (type === "monthly") {
      if (monthValue !== undefined)
        formData.append("month", String(monthValue));
      if (yearValue !== undefined) formData.append("year", String(yearValue));
      formData.append("textFi", textFiValue);
      formData.append("textEn", textEnValue);
    } else if (textFiValue || textEnValue) {
      formData.append("textFi", textFiValue);
      formData.append("textEn", textEnValue);
    }
      ...
      <Form onSubmit={addPicture} encType="multipart/form-data">
        ...
        <div className="form__group input">
          ...
          <select
            ...
            required
            value={selectedType}
            onChange={(event) => setSelectedType(typeCheck(event.target.value))}
          >
        ...
        <div className="buttonDiv">
          ...
          {selectedType != "monthly" && selectedType != undefined && (
            <Button
              variant="primary"
