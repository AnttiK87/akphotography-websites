...
import type { Metadata } from "../../types/pictureTypes";

type ImageMetadataProps = {
  src: string;
};

const ImageMetadata = ({ src }: ImageMetadataProps) => {
  ...
  const parseShutterSpeed = (shutterSpeed: string) => {
    const parsedShutterSpeed = Math.round(1 / Number(shutterSpeed));
    if (parsedShutterSpeed > 1) {
      return <span> 1/{parsedShutterSpeed} s</span>;
    } else {
      return <span> {shutterSpeed} s</span>;
    }
  };
      ...
      metadata?.DateTimeOriginal ? (
        <div>
          ...
          <div className="iconExifText">
            ...
            />
            {parseShutterSpeed(metadata.ExposureTime)}
          </div>
          <div className="iconExifText">
